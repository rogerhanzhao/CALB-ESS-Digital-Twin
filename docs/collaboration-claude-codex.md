...
## 10. 持续推进机制（Continuous delivery loop）

项目目标不是追求通用仿真平台，而是交付符合 Alex.Z 实际岗位工作流的工程工具：
新产品或新方案建立档案，导入并审核测试数据，标定 PyBaMM / 半经验模型，执行少量
标准工况，保存不可变结果版本，再做版本对比、工程审核和质保边界分析。

为避免 CODEX 与 Claude 同时等待，默认执行权固定如下：

1. **CODEX 是推进方**：从已批准设计与未完成里程碑中选择下一个最小可验证切片，
   实现、测试、提交 Draft PR，并记录事实状态。
2. **Claude 是异步审核方**：审核已提交的边界条件、契约、数据完整性、迁移和回归风险；
   审核意见不阻塞 CODEX 开始下一个不冲突的切片。
3. 若设计阶段目标已完成而下一产品功能不明确，CODEX 自动转入维护队列：复现缺陷、
   审计安全与权限、补测试、检查性能和资源上限、清理演示/真实边界、更新运行文档。
4. 只有需要 Alex.Z 作出会改变产品口径、质保责任、数据授权或基础设施成本的决定时，
   双方才暂停相应切片；其他不冲突工作继续。
5. 每个循环都以 GitHub 的 commit、Draft PR、CI 与 review thread 为共享状态；定时检查
   只是发现新状态的手段，不能替代任何一方主动选择并执行工作。

因此，“30 分钟后检查”不代表停止开发。没有新审核时，CODEX 继续当前里程碑；有新
审核时，先处理高风险 finding，再继续。Claude 没有新任务时审核最新可验证提交，而不
要求 CODEX 先等待其动作。
