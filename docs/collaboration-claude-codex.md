...
2026-08-13 起加入第三个智能体 **WORKBUDDY**，它不占据上表任何一行 ——
它是**审计方（Agent-C）**，边界另见 §11。上表只分配"实现"与"互审"两个角色，
审计方独立于两者。
...
**开工第一件事**：读 `docs/agent-startup-checklist.md`。
那份文件记录上一轮交叉审核的未决结论与当轮禁止动作，三方通用，比本文件更新得更频繁。
本文件定规则，那份文件定"这一轮具体该做什么、不该做什么"。
...
§10.1–§10.5 的全部约束**按智能体分别计算,不按角色豁免**。审计方（§11）同样受
"一条工作线一个分支""同时最多 3 个在审 PR""单 PR ≤800 行"约束 —— 审计不是开分支的许可证。

---

## 11. 第三智能体 WORKBUDDY（Agent-C，审计方）

> **状态：暂行（provisional），待 Alex.Z 确认后转正。**
>
> §6.3 与 §6.4 规定，参与方与权限这类事项**不得由任一智能体单方面决定**。
> 本节由 Claude 起草，用途是让 WORKBUDDY 在首轮已经发生越权之后**立刻有边界可依**——
> 边界收紧（白名单 + 禁止改实现）在待批期间是安全的默认值，比无规则可依更安全。
>
> 但其中两项属于**长期授权**，需要 Alex.Z 明确批准才生效：
> ①  Agent-C 作为常设参与方的地位；②  `workbuddy/*` 分支命名空间与 `tests/` 写入权。
>
> **待批期间 WORKBUDDY 的实际权限，取 §11.2 白名单与下表的交集**：
>
> | 动作 | 待批期间 | Alex.Z 批准后 |
> |---|---|---|
> | PR 评论、issue 评论提 finding | 允许 | 允许 |
> | `docs/audit-*.md` 审计报告 | 允许，但只能推到**已存在的** `audit/docs-governance-f2-f7` 分支 | 允许，改用 `workbuddy/*` |
> | 新建 `workbuddy/*` 分支 | **不允许**（该命名空间尚未生效） | 允许 |
> | 在 `tests/` 下新增测试 | **不允许** | 允许 |
> | 改实现代码 / 共有文档 | 不允许 | 不允许 |
>
> 即：待批期间只放行"评论 + 审计报告"，且走已有分支，不开新命名空间。
> §11.1 表格中的 `workbuddy/*` 与 §11.2 白名单第 2 项（`tests/`）**在批准前不生效**。
> 任一方不得援引本节主张 Agent-C 的常设地位。

2026-08-13 加入。它的存在是为了打破 §0 风险 2（同质化盲区）：两个互审的智能体
可能同时看不见同一个错误，第三方从**仓库外部视角**做审计，不参与实现。

首次实测有效 —— WORKBUDDY 的 PR #48 与 CODEX 对它的交叉审核**各自发现了对方漏掉的问题**，
机制成立。但同一轮也暴露出边界未定义导致的越权，本节即由此产生。

### 11.1 角色定义

| 项 | 规定 |
|---|---|
| 代号 | Agent-C / WORKBUDDY |
| 分支前缀 | **`workbuddy/<主题>`** —— 不再使用 `audit/*`（§2.1 未定义该前缀） |
| 定位 | 审计方。**不是** §1 表格里的 Primary，也**不是**任何区域的 Reviewer-of-record |
| 产出 | 审计报告（文档）+ finding（PR 评论）+ 测试。**不改任何实现代码** |

### 11.2 可写区域（白名单，其余一律只读）

WORKBUDDY **只能**写这三类路径：

1. `docs/audit-*.md` —— 它自己的审计报告，唯一写者是它自己；
2. `tests/` 下**新增**的测试文件 —— 只增不改,不得修改他人已有断言（同 §3 L3 规定）；
3. 它自己 PR 的描述与评论。

**明确禁止**（首轮 PR #48 实际发生过的越权）：

- `.github/workflows/*`、`pyproject.toml`、根配置 —— §1 归 Agent-B Primary；
- `src/`、`compute/`、`web/` 的任何实现代码；
- `README.md`、`CHANGELOG.md`、`docs/design-review.md` 等**共有文档的直接改写**。

共有文档确有漂移时，WORKBUDDY 的正确动作是**提 finding，指名 Primary 去改**，
而不是自己动手 —— 理由与 §1 铁律一致：谁写的谁负责，审计方自己改就没人审它了。

### 11.3 CHANGELOG 的写入权

§2.3 规定 A 写「计算面」、B 写「控制面」。**WORKBUDDY 两个小节都不写。**
审计不产生用户可见变更，因此不进 CHANGELOG；审计结论进 `docs/audit-*.md`。

（PR #48 由 WORKBUDDY 补写了「Compute plane」小节 —— 内容大体属实，但那是 Agent-A 的格子。
正确做法是提 finding 让 CODEX 补。）

### 11.4 审计报告必须先入库

**审计报告是 finding 的唯一依据，必须与引用它的 PR 同时可见。**

PR #48 的描述通篇引用 `docs/third-party-audit.md`，而该文件**从未提交到任何分支**。
后果是 F1–F7 全部无法复核：审核者看不到 F1 是什么、为什么编号从 F2 开始、
每条 finding 的原始证据是什么。

规定：引用审计报告的 PR，**报告本身必须在同一个 PR 里**，或已经合并进 `main`。
引用一份不存在的文档，等同于没有依据。

### 11.5 finding 编号连续性

审计 finding 一经编号即**不得跳号**。若某条被撤回，保留编号并标注
`F1 — 撤回，理由：…`。跳号会让下一个审核者无法判断是"漏做了"还是"故意不做"。

### 11.6 提 finding 前必须查在审 PR

**审计的对象是 `main` 加上所有在审 PR，不只是 `main`。**

PR #48 的 F6 删除了对 `docs/physical-world-operating-model.md` 的引用，理由是"悬空引用"。
