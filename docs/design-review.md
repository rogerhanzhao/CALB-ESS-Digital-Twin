...
- 审核日期：2026-08-09
- 文档状态：**评审中** — CODEX 交叉审核进行中（#2）。PR #1 合并后转为 V0.2 的正式实施基线。
...
| # | 问题 | 影响 |
|---|---|---|
| Q1 | 计算面部署在何处？（自建 K8s / 云容器服务 / 现有 HPC） | 决定最终的队列与镜像方案。**不阻塞 M1**：先用可替换的本地 Docker adapter 打通工程闭环，云端目标待决策后落位 |
| Q2 | 是否有可用的真实老化数据集及其规模？ | **阻塞 M2** 的真实标定与可信 SOH 交付。**不阻塞 M1**：M1 用显式标注的 synthetic fixture / stub result 完成端到端闭环 |
