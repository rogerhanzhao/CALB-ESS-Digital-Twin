...
export async function PUT(request: Request, { params }: Context) {
  ...
  if (!isArtifactKind(kind)) {
    return Response.json({ error: "unsupported standard-study artifact" }, { status: 400 });
  }
  ...
  const objectKey = resultObjectKey(id, kind);
  await env.STUDY_ARTIFACTS.put(objectKey, content, {
    httpMetadata: { contentType: "application/json" },
    customMetadata: { runId: id, kind, sha256: checksum },
  });
