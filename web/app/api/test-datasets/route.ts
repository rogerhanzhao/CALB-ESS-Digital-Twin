...
import { MAX_SOURCE_DATASET_BYTES, validateDatasetSource } from "../../../lib/dataset-source";
...
export async function POST(request: Request) {
  ...
  const db = getDb();
  const [existing] = await db.select().from(testDatasets).where(and(
    eq(testDatasets.userId, owner), eq(testDatasets.idempotencyKey, idempotency.value),
  )).limit(1);
  if (existing) return Response.json({ dataset: publicDataset(existing) });
  const declaredLength = Number(request.headers.get("content-length"));
  if (Number.isFinite(declaredLength) && declaredLength > MAX_SOURCE_DATASET_BYTES + 1024 * 1024) {
    return Response.json({ error: "multipart upload exceeds the V0.2 request limit" }, { status: 413 });
  }
  let form: FormData;
  ...
  if (source.size > MAX_SOURCE_DATASET_BYTES) {
    return Response.json({ error: "source file exceeds the 25 MiB V0.2 limit" }, { status: 413 });
  }
  const payload: Record<string, unknown> = Object.fromEntries([...form.entries()].filter(([key]) => key !== "sourceFile"));
  ...
  const sourceValidation = validateDatasetSource(source.name, new Uint8Array(content));
  if (!sourceValidation.ok) return Response.json({ error: sourceValidation.error }, { status: 400 });
  ...
  const dataset = {
    id: datasetId, userId: owner, sampleId: sample.id, schemaVersion: "V0.2", ...input,
...
function publicDataset(dataset: typeof testDatasets.$inferSelect | typeof testDatasets.$inferInsert) {
  const { storageUri: _storageUri, idempotencyKey: _idempotencyKey, ...safe } = dataset;
  void _storageUri; void _idempotencyKey;
  return { ...safe, sourceHref: `/api/test-datasets/${encodeURIComponent(dataset.id!)}/source` };
}
