 ...
 *
 * `chargeRateC` and `dischargeRateC` are optional because no scenario currently carries
 * them -- `ScenarioInput` in the contract has no C-rate field. They are declared here so
 * the gap is visible and typed rather than silently skipped.
 */
...
export function evaluateValidityEnvelope(
  ...
  checkRange("ambient temperature", duty.ambientTemperatureC, envelope.temperatureMinC, envelope.temperatureMaxC, breaches, unevaluated);
  ...
  if (envelope.socMin !== null && duty.socWindowMin < envelope.socMin - TOLERANCE) {
    breaches.push(`SOC window floor ${duty.socWindowMin} is below the calibrated minimum ${envelope.socMin}`);
  }
  if (envelope.socMax !== null && duty.socWindowMax > envelope.socMax + TOLERANCE) {
    breaches.push(`SOC window ceiling ${duty.socWindowMax} is above the calibrated maximum ${envelope.socMax}`);
  }
  ...
  checkCeiling("calendar exposure in days", calendarDays, envelope.maxCalendarDays, breaches);

  const cycles = duty.cyclesPerDay * calendarDays;
  checkCeiling("cycle count", cycles, envelope.maxCycles, breaches);
  ...
  checkCeiling("equivalent full cycles", equivalentFullCycles, envelope.maxEquivalentFullCycles, breaches);
